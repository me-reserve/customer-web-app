import 'package:me_reserve_bem_estar/feature/search/model/filter_model.dart';
import 'package:me_reserve_bem_estar/feature/search/model/search_service_model.dart';
  ...
  Future<void> searchData({required String query, required offset, bool shouldUpdate = true, bool reload = true}) async {
   ...
   if(shouldUpdate) {
    ...
    Response response = await searchRepo.getSearchData(
      ...
      minPrice: _filteredMinPrice ?? 0, maxPrice: _filteredMaxPrice ?? _serviceModel?.content?.filteredMaxPrice ?? 0, rating: _selectedRating, categoryIdes: _selectedCategoryId,
    );
